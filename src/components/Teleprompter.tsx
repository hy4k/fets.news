...
export const Teleprompter: React.FC<TeleprompterProps> = ({
  isOpen,
  onClose,
  defaultScript = `GOOD EVENING, I'M LIVE IN STUDIO A. WE ARE TRACKING RAPID DEVELOPMENTS ACROSS GLOBAL FINANCIAL CENTERS AND CAPITAL BUREAUS TONIGHT.

A PIVOTAL AGREEMENT HAS BEEN REACHED IN GENEVA, SPARKING IMMEDIATE REACTIONS FROM TRADING DESKS IN NEW YORK, LONDON, AND TOKYO.

OUR SATELLITE NETWORK IS FULLY LINKED WITH CORRESPONDENTS STANDING BY ON THE GROUND.

WE CROSS LIVE FIRST TO OUR SENIOR CORRESPONDENT FOR THE IMMEDIATE TAKEAWAYS. TAKE IT AWAY.`,
}) => {
  ...
  return (
    ...
    >
      <div className="w-full max-w-3xl bg-[#090e1c] border-2 border-indigo-500/60 rounded-xl shadow-2xl flex flex-col h-[85vh] overflow-hidden">
        ...
        {/* Footer Note */}
        <div className="p-2 bg-slate-950 border-t border-slate-800 text-center font-tech text-[11px] text-slate-500">
          STUDIO A PROMPTER • SCRIPT READOUT ENGINE • CONTROL SPEED VIA NUMBER KEYS OR BUTTONS
        </div>
      </div>
    </div>
  );
